...
export default function PitchDraftAssistant({ displayName, clientName, existingDraft, onApplyDraft, sidebar = false }: Props) {
  ...
  const startSession = async (selectedMode: Mode) => {
    ...
    const res = await fetch('/api/ai/pitch-draft', {
      method: 'POST',
    ...
    const firstUserMsg: Message = selectedMode === 'review' && existingDraft
      ? { role: 'user', content: `添削をお願いします。現在の営業メッセージは以下のとおりです。\n\n${existingDraft}` }
      : { role: 'user', content: 'こんにちは。営業メッセージを一緒に作成していただけますか？' }
  ...
  function renderMessageText(text: string): string {
    return text.replace(/```pitch\n[\s\S]*?```/g, '').trim()
  }

  function extractDraftFromMessage(text: string): string | null {
    const matches = Array.from(text.matchAll(/```pitch\n([\s\S]*?)```/g))
    ...
    <>
      <div style={{
        ...
        <div>
          <p style={{ margin: 0, fontWeight: '800', fontSize: '16px', color: 'var(--c-text)', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Sparkles size={16} aria-hidden /> AI 営業メッセージアシスタント
          </p>
      ...
      {!mode ? (
        <div style={{ padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            <button type="button" onClick={() => startSession('review')} disabled={!existingDraft.trim()}
              ...
              <p style={{ margin: 0, fontSize: '13px', color: 'var(--c-text-3)', lineHeight: '1.6' }}>
                {existingDraft.trim() ? '入力済みの文章を確認し、改善点を提案します' : 'メッセージを入力してから利用できます'}
              </p>
            </button>
          </div>
          <div style={{ padding: '12px 14px', borderRadius: '10px', background: 'var(--c-input-bg-2)', border: '1px solid var(--c-border)' }}>
            <p style={{ margin: 0, fontSize: '12px', color: 'var(--c-text-4)', lineHeight: '1.7', display: 'flex', alignItems: 'flex-start', gap: '5px' }}>
              ...
              <span>強み・連絡理由が伝わるか確認します。<br />外部連絡先への誘導は原則禁止です。</span>
            </p>
          </div>
        </div>
      ) : (
        <>
                ...
                <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: isUser ? 'flex-end' : 'flex-start', gap: '8px' }}>
                  ...
                  {inlineDraft && (
                    <div style={{ maxWidth: '90%', padding: '14px 16px', borderRadius: '14px', background: 'var(--c-accent-a08)', border: '1px solid var(--c-border)' }}>
                      <p style={{ margin: '0 0 8px', fontSize: '11px', fontWeight: '700', color: 'rgb(var(--brand-rgb))', letterSpacing: '0.06em' }}>営業メッセージ（案）</p>
          ...
          {latestDraft && (
            <div style={{ padding: '10px 16px', borderTop: '1px solid var(--c-border)', background: 'var(--c-accent-a06)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
              <p style={{ margin: 0, fontSize: '12px', color: 'rgb(var(--brand-rgb))', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <Sparkles size={13} aria-hidden /> 営業メッセージの案があります
              </p>
          ...
          <div style={{ padding: '12px 16px', borderTop: '1px solid var(--c-border)', display: 'flex', gap: '8px', flexShrink: 0 }}>
            <textarea
              ...
              rows={2}
              disabled={loading}
              style={{ flex: 1, padding: '10px 12px', borderRadius: '10px', resize: 'none', border: '1px solid var(--c-border)', background: 'var(--c-input-bg)', color: 'var(--c-text)', fontSize: '14px', outline: 'none', lineHeight: '1.5' }}
            />
            <button type="button" onClick={sendMessage} disabled={loading || !input.trim()}
              style={{ ...btnBase, alignSelf: 'flex-end', background: loading || !input.trim() ? 'var(--c-accent-a20)' : 'rgb(var(--brand-rgb))', color: '#fff', padding: '10px 16px', cursor: loading || !input.trim() ? 'not-allowed' : 'pointer' }}>
              送信
            </button>
          </div>
        </>
  ...
  return (
    <>
      <button type="button" onClick={() => { setOpen(true); reset() }}
        style={{ width: '100%', padding: '13px', borderRadius: '12px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', border: '1px solid var(--c-border-2)', background: 'var(--c-accent-a08)', color: 'rgb(var(--brand-rgb))' }}>
        <Sparkles size={15} aria-hidden /> AIで営業メッセージを作成・添削する
      </button>
