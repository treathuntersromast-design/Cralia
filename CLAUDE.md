...
.envの中身は会話内に絶対に展開しないでください。これは徹底してください。
...
## Goal
このプロジェクトでは、洗練された、余白の美しい、モダンで信頼感のあるWebデザインを作る。

## Design Principles
- 余白を広めに取り、詰め込みすぎない
- 文字サイズに明確な階層を作る
- 見出し、本文、補足テキストのコントラストを明確にする
- ボタンやカードは角丸、影、境界線を控えめに使う
- 色数は少なくする。基本は背景色、本文色、アクセント色の3系統
- スマホ表示を必ず確認する
- アニメーションは控えめにし、意味がある場合だけ使う

## UI Quality Checklist
実装後、必ず以下を確認する。

1. ファーストビューで何のサイトか3秒で分かるか
2. CTAボタンが目立つか
3. 余白が窮屈ではないか
4. PC、タブレット、スマホで崩れていないか
5. 文字が小さすぎないか
6. 色のコントラストが弱すぎないか
7. ボタン、カード、フォームのデザインが統一されているか
8. 不要な装飾がないか

## Claude Behavior
- いきなり実装せず、まずデザイン方針を短く提案する
- 実装後は、改善点を自分でレビューする
- UI変更後は `npm run build` を実行し、エラーがないか確認する
- Tailwindのクラスは読みやすく整理する
- コンポーネントは再利用しやすく分ける
...
ファイルを修正した場合、プロジェクトルートの以下の設計書 Excel の**該当箇所を併せて修正**する。

| 設計書 | 更新対象の例 |
|---|---|
| `要件定義書.xlsx` | 機能要件・非機能要件の追加・変更 |
| `基本設計書.xlsx` | DB スキーマ、API 一覧、機能概要の変更 |
| `詳細設計書.xlsx` | テーブル定義、API 詳細仕様、画面遷移の変更 |
| `画面設計書.xlsx` | 画面レイアウト、ボタン・フロー、画面遷移図の変更 |

- **リリース前は設計書のバージョンを更新しない**（バージョン番号・日付を変えない）。
- 設計書の更新には Python スクリプト（openpyxl）を使用し、実行後はスクリプトを削除する。

### テストを行った場合のテスト仕様書更新

テストを実施した場合、`テスト仕様書.xlsx` を更新する。

- **リリース前は再テスト結果を前回結果に上書きする形で記載する**（行を追加しない）。
  - 例: 前回 NG → 修正後 OK → 該当行の「結果」列を OK に、「修正要否・修正内容」を「修正済」に上書き。
- 新機能・新テストケースは新規行として追加する。
- テスト仕様書の更新も Python スクリプトで行い、実行後はスクリプトを削除する。

### 操作説明書の更新

新機能の追加・画面変更・操作フロー変更を行った場合、`Cralia操作説明書.xlsx` の**該当箇所を併せて修正**する。

| 説明書シート | 更新対象の例 |
|---|---|
| `サービス概要` | エスクロー仕様・手数料体系の変更 |
| `依頼者向け操作` | 依頼・決済フローの変更 |
| `クリエイター向け操作` | 受注・報酬確認フローの変更 |
| `管理者向け操作` | 管理画面の操作方法・メニュー追加 |
| `イベント管理` | イベント機能の変更 |
| `FAQ・トラブル` | よくある問い合わせの追加 |
| `画面一覧` | 新規画面の追加・URL 変更 |

- 説明書の更新も Python スクリプト（openpyxl）を使用し、実行後はスクリプトを削除する。
- 画面一覧シートのスクリーンショット列は手動で貼り付ける（Claude は自動キャプチャ不可）。

### 機能追加時の手動テスト仕様書更新

**新しい機能・画面・API を追加・変更した場合は、必ず `手動テスト仕様書.xlsx` を併せて更新する。**

- 対象ファイル: プロジェクトルートの `手動テスト仕様書.xlsx`（`テスト仕様書.xlsx` とは別ファイル）
- 追加・変更した機能に対応するテストケースを「手動テスト仕様書」シートに新規行として追加する。
- テストケースには以下を必ず記載する。

| 列 | 記載内容 |
|---|---|
| No. | `TC-{区分}-{連番}` 形式（例: TC-ORD-012） |
| 区分 | 機能の分類（認証 / 依頼フロー / AI依頼文 など） |
| テスト項目 | テスト名（何を確認するか） |
| 前提条件 | テスト実施に必要な状態・アカウント |
| 具体的なアクション | 番号付きの操作手順 |
| 期待結果 | 正常動作の確認内容 |
| 備考 | 注意点・依存するシードデータなど |

- 更新には Python スクリプト（openpyxl）を使用し、実行後はスクリプトを削除する。
- エスクロー決済など将来実装予定の機能は「エスクローシナリオ」シートに追記する。
