...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (safeDisplayName)     contextLines.push(`クリエイターの名前: ${safeDisplayName}`)
    if (safeExistingDraft)   contextLines.push(`現在の仕事募集文（添削対象）:\n---\n${safeExistingDraft}\n---`)
    ...
    await logError({ endpoint: 'ai/creator-listing-draft', message, stack, userId })
